...
- **Store** (`src/store/`): Zustand store with `persist` middleware. Shape: `{ drinks: DrinkEntry[], settings: Settings, customPresets: CustomPreset[], schedules: DrinkSchedule[], ...actions }`. Schema version 7 (v1->v2: targetBedtime, v2->v3: customPresets, v3->v4: metabolismMode, covariates, v4->v5: schedules, v5->v6: hiddenPresetIds, showResearchThresholds, v6->v7: timestamp->startedAt, endedAt, durationMinutes).
- **Types** (`src/engine/types.ts`): `DrinkEntry` (id, name, caffeineMg, startedAt, endedAt, presetId), `Settings` (halfLifeHours, thresholdMg, targetBedtime, metabolismMode, covariates, hiddenPresetIds, showResearchThresholds, caffeineSensitivity, thresholdSource), `MetabolismMode` ('simple' | 'advanced'), `CaffeineSensitivity` ('low' | 'normal' | 'high'), `ThresholdSource` ('manual' | 'autonomic' | 'deep_sleep'), `ThresholdZone` ('clear' | 'autonomic' | 'sleep_disruption'), `CovariateSettings` (weight, weightUnit, sex, smoking, oralContraceptives, pregnancyTrimester, liverDisease, cyp1a2Genotype, cyp1a2Inhibitor), `DEFAULT_COVARIATES`, `CustomPreset` (id, name, caffeineMg, durationMinutes, calculatorParams?), `CalculatorParams` (brewMethod, beanType, beanCaffeinePercent?, doseG, grindSize, waterTempC), `BrewMethod`, `BeanType`, `GrindSize`, `BrewMethodConfig`, `CalculatorInput`, `CalculatorResult`, `DrinkSchedule` (id, presetId, name, caffeineMg, timeOfDay, repeatDays, paused, lastRunDate), `CurfewResult`, `DrinkCurvePoint`.
